...
// ==========================================
// LOGIN SCREEN (WITH OTP RESET SYSTEM)
// ==========================================
...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Future<void> _sendResetOtp() async {
    if (_emailController.text.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Pehle apna email box me bharein!")));
      return;
    }
    setState(() => _isLoading = true);
    try {
      ...
      setState(() {
        _isResetMode = true; // Show OTP fields
      });
      if(mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text("6-Digit Code sent! Apne email ka Inbox/Spam check karein."), 
  ...
  Future<void> _verifyOtpAndSavePassword() async {
    ...
    if (_newResetPasswordController.text.length < 6) {
    ...
    try {
      ...
      await Supabase.instance.client.auth.updateUser(
        UserAttributes(password: _newResetPasswordController.text.trim()),
      );
      ...
      setState(() {
        _isResetMode = false;
        _isLoginMode = true;
        _passwordController.text = _newResetPasswordController.text; // Auto-fill for login
        _otpController.clear();
        _newResetPasswordController.clear();
      });
      
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text("Password changed successfully! You can now login."), 
          backgroundColor: Colors.green,
          duration: Duration(seconds: 4),
        ));
      }
    } catch (e) {
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Invalid OTP or Error. Code match nahi hua!"), backgroundColor: Colors.red));
    } finally {
      if (mounted) setState(() => _isLoading = false);
    }
  }

  Widget _buildInputField({required String label, required String hint, required IconData prefixIcon, required TextEditingController controller, bool isPassword = false, TextInputType inputType = TextInputType.text}) {
    return Column(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    // 🔥 RESET PASSWORD OTP MODE 🔥
                      ...
                      const SizedBox(height: 20),
                      _buildInputField(label: "Enter 6-Digit OTP", hint: "123456", prefixIcon: Icons.message, controller: _otpController, inputType: TextInputType.number),
                      const SizedBox(height: 20),
                      ...
                      Container(
                        ...
                        child: ElevatedButton(
                          style: ElevatedButton.styleFrom(backgroundColor: Colors.transparent, shadowColor: Colors.transparent, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))),
                          onPressed: _isLoading ? null : _verifyOtpAndSavePassword,
                          child: _isLoading ? const SizedBox(height: 24, width: 24, child: CircularProgressIndicator(color: Colors.white, strokeWidth: 2)) : const Text("Update Password", style: TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.bold, letterSpacing: 1)),
                        ),
                      ),
                      const SizedBox(height: 30),
                      GestureDetector(
                        onTap: () => setState(() => _isResetMode = false),
                        child: const Text("Back to Login", style: TextStyle(color: Colors.white54, fontSize: 14, fontWeight: FontWeight.bold, decoration: TextDecoration.underline)),
                      ),
                    ],
    ...
    try {
      ...
      for (var item in animeResponse) {
        ...
        if (item['dub_color'] != null && item['dub_color'].toString().isNotEmpty) {
          try {
            String hex = item['dub_color'].toString().replaceAll('#', '');
            if(hex.length == 6) hex = 'FF$hex';
            dubColorParsed = Color(int.parse(hex, radix: 16));
          } catch(e){}
        }
                    ...
                    return GestureDetector(
                      onTap: () { 
                        if (isCustom || hero['anime_id'] == null) {
                          ...
                          try {
                            final linkedAnime = animeListNotifier.value.firstWhere((a) => a.id == hero['anime_id']);
              ...
              builder: (context, allAnime, child) {
                ...
                return Column(
                  children: [
                    ...
                    if (suspenseList.isNotEmpty) _buildPortraitSection(context, "Suspense", null, null, suspenseList),

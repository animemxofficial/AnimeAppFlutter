...
// ==========================================
// APP INITIALIZATION & SUPABASE SETUP
// ==========================================
void main() async {
  WidgetsFlutterBinding.ensureInitialized();

  // 👉 BHAI YAHAN APNI SUPABASE URL AUR ANON KEY DAALNA 👈
  await Supabase.initialize(
    url: 'https://yngzfgfpyufusrbitagl.supabase.co',          // Example: 'https://xyzabcdef.supabase.co'
    anonKey: 'sb_publishable_6BD0moEpOnUTfihbRUpdOQ_U2gJCH5U', // Example: 'eyJhbGciOiJIUzI1NiIsInR5c...'
  );
...
// ==========================================
// SUPABASE AUTH SCREEN (LOGIN / SIGNUP)
// ==========================================
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              Text(_isLoginMode ? "Welcome Back!" : "Create an Account", style: const TextStyle(color: Colors.white54, fontSize: 16)),
                    ...
                    children:[
                      ...
                      Positioned(
                        ...
                        child: Container(
                          ...
                          child: Text("${i + 1}/${sliderItems.length}", style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
                        ),
                      ),
                      Positioned(
                        bottom: 20, left: 15,
                        child: Column(
                          ...
                          children:[
                            SizedBox(
                              ...
                              child: Text(anime.title.toUpperCase(), style: const TextStyle(color: Colors.white, fontSize: 20, fontWeight: FontWeight.w900, letterSpacing: 1), maxLines: 1, overflow: TextOverflow.ellipsis),
                            ),
                            const SizedBox(height: 8),
                            Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4), decoration: BoxDecoration(color: tagColor, borderRadius: BorderRadius.circular(4)), child: Text(tag, style: const TextStyle(color: Colors.white, fontSize: 10, fontWeight: FontWeight.bold))),
                          ],
                        ),
                      )
                    ],
              ...
              child: Row(
                ...
                children:[
                  Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children:[
                      const Text("Top Picks for You", style: TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.bold)),
                      const SizedBox(height: 4),
                      Text("New episodes available now!", style: TextStyle(color: Colors.grey[400], fontSize: 12)),
                    ],
                  ),
                  ElevatedButton(
                    ...
                    onPressed: () { Navigator.push(context, MaterialPageRoute(builder: (_) => DetailsPage(anime: animeData[0]))); },
      ...
      children:[
        Padding(
          ...
          child: Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children:[
              Row(children:[Text(title, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w900, color: Colors.white)), if (icon != null) ...[const SizedBox(width: 6), Icon(icon, color: iconColor, size: 20)]]),
              GestureDetector(onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => SeeAllCategoryPage(title: title, animeList: list))), child: const Text("See All", style: TextStyle(color: Colors.orange, fontWeight: FontWeight.bold, fontSize: 13))),
            ],
          ),
        ),
        SizedBox(
          ...
          child: ListView.builder(
            ...
            itemBuilder: (context, index) { return PortraitTextBelowCard(anime: list[index]); },
          ),
        ),
        const SizedBox(height: 16),
      ],
      ...
      children:[
        Padding(
          ...
          child: Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              Row(children:[Text(title, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w900, color: Colors.white)), const SizedBox(width: 6), Icon(icon, color: iconColor, size: 20)]),
              GestureDetector(onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => SeeAllCategoryPage(title: title, animeList: list))), child: const Text("See All", style: TextStyle(color: Colors.orange, fontWeight: FontWeight.bold, fontSize: 13))),
            ],
          ),
        ),
      ...
      children:[
        Padding(
          ...
          child: Row(
            ...
            children: [
              Row(children:[Text(title, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w900, color: Colors.white)), if (icon != null) ...[const SizedBox(width: 6), Icon(icon, color: iconColor, size: 20)]]),
              GestureDetector(onTap: () { List<Anime> listToPass = isCW ? cwList!.map((cw) => cw.anime).toList() : animeList!; Navigator.push(context, MaterialPageRoute(builder: (_) => SeeAllCategoryPage(title: title, animeList: listToPass))); }, child: const Text("See All", style: TextStyle(color: Colors.orange, fontWeight: FontWeight.bold, fontSize: 13))),
            ],
          ),
        ),
        SizedBox(
          ...
          child: ListView.builder(
            ...
            itemBuilder: (context, index) {
              if (isCW) { return CWAnimeCard(item: cwList![index]); } else { return ThumbnailLatestCard(anime: animeList![index]); }
            },
          ),
        ),
        const SizedBox(height: 16),
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children:[
            Expanded(child: ClipRRect(borderRadius: BorderRadius.circular(12), child: Image.network(anime.image, fit: BoxFit.cover, width: double.infinity))),
            const SizedBox(height: 8),
            ...
            Text(anime.genre, style: const TextStyle(color: Colors.grey, fontSize: 11), maxLines: 1, overflow: TextOverflow.ellipsis),
          ],
        ),
      ),
    );
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        duration: const Duration(milliseconds: 150), curve: Curves.easeOut, transform: Matrix4.identity()..scale(_isTapped ? 0.96 : 1.0),
        ...
        child: Column(
          ...
          children:[
            ...
            const SizedBox(height: 8),
            Text(widget.item.anime.title, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 13), maxLines: 1, overflow: TextOverflow.ellipsis),
            Text("Episode ${widget.item.episodeIndex + 1}", style: const TextStyle(color: Colors.grey, fontSize: 11)),
          ],
        ),
      ),
    );
...
class _DetailsPageState extends State<DetailsPage> {
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.anime.seasonsList.isEmpty) {
      return Scaffold(appBar: AppBar(backgroundColor: Colors.black, title: Text(widget.anime.title)), body: const Center(child: Text("Episodes Coming Soon!", style: TextStyle(color: Colors.white))));
    }
            ...
            flexibleSpace: FlexibleSpaceBar(
              background: Stack(
                ...
                children:[
                  ...
                  Container(decoration: BoxDecoration(gradient: LinearGradient(colors:[darkBg, darkBg.withOpacity(0.5), Colors.transparent], stops: const [0.0, 0.4, 1.0], begin: Alignment.bottomCenter, end: Alignment.topCenter))),
                ],
              ),
            ),
          ),
                ...
                children:[
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton.icon(
                      ...
                      onPressed: () {
                        if (episodesList.isNotEmpty) Navigator.push(context, MaterialPageRoute(builder: (context) => VideoPlayerPage(anime: widget.anime, seasonIndex: _selectedSeasonIndex, episodeIndex: 0)));
                      },
                      icon: const Icon(Icons.play_arrow_rounded, color: Colors.white, size: 28),
                      label: const Text("Play Season 1", style: TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.bold)),
                    ),
                  ),
...
class MyListScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: Colors.black,
      appBar: AppBar(title: const Text("My List", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)), backgroundColor: Colors.black, elevation: 0),
      body: ValueListenableBuilder<List<SavedEpisode>>(
        valueListenable: myListNotifier,
        builder: (context, savedList, child) {
          if (savedList.isEmpty) return const Center(child: Text("Your list is empty.\nSave episodes to watch later!", textAlign: TextAlign.center, style: TextStyle(color: Colors.white54, fontSize: 16)));
          return ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              return GestureDetector(
                onTap: () => Navigator.push(context, MaterialPageRoute(builder: (context) => VideoPlayerPage(anime: item.anime, seasonIndex: item.seasonIndex, episodeIndex: item.episodeIndex))),
                child: Container(
                  margin: const EdgeInsets.only(bottom: 16), decoration: BoxDecoration(color: const Color(0xFF1A1A1A), borderRadius: BorderRadius.circular(12), border: Border.all(color: Colors.white12)),
                  child: Row(
                    children:[
                      SizedBox(width: 140, height: 90, child: ClipRRect(borderRadius: const BorderRadius.only(topLeft: Radius.circular(12), bottomLeft: Radius.circular(12)), child: Stack(fit: StackFit.expand, children:[Image.network(ep.image, fit: BoxFit.cover), Container(color: Colors.black38), const Center(child: Icon(Icons.play_circle_fill, color: Colors.white, size: 36))]))),
                      const SizedBox(width: 12),
                      Expanded(child: Padding(padding: const EdgeInsets.symmetric(vertical: 10, horizontal: 8), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children:[Text(item.anime.title, style: const TextStyle(color: Colors.orange, fontWeight: FontWeight.bold, fontSize: 12), maxLines: 1, overflow: TextOverflow.ellipsis), const SizedBox(height: 4), Text(ep.title, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 15), maxLines: 1, overflow: TextOverflow.ellipsis), const SizedBox(height: 6), Text("${item.anime.seasonsList[item.seasonIndex].name} | Episode ${item.episodeIndex + 1}", style: const TextStyle(color: Colors.white54, fontSize: 12))]))),
                      IconButton(icon: const Icon(Icons.bookmark, color: Colors.orange), onPressed: () { final list = List<SavedEpisode>.from(myListNotifier.value); list.removeAt(index); myListNotifier.value = list; }),
                    ],
                  ),
                ),
              );
...
class _VideoPlayerPageState extends State<VideoPlayerPage> {
  ...
  void initState() {
    ...
    _controller = VideoPlayerController.networkUrl(Uri.parse(ep.videoUrl), videoPlayerOptions: VideoPlayerOptions(mixWithOthers: true))..initialize().then((_) { if (widget.startPosition != null) { _controller.seekTo(widget.startPosition!); } setState(() {}); _controller.play(); });
  }
  ...
  void _toggleSave() {
    ...
    myListNotifier.value = list; setState(() {});
  }
        ...
        if (_showControls) GestureDetector(
          ...
          child: Container(
            ...
            child: Column(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children:[
                Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children:[IconButton(icon: const Icon(Icons.arrow_back, color: Colors.white, size: 28), onPressed: () { if (_isFullScreen) _toggleFullScreen(); Navigator.pop(context); }), Row(children:[IconButton(icon: const Icon(Icons.cast, color: Colors.white), onPressed: () {}), IconButton(icon: Icon(_isFullScreen ? Icons.fullscreen_exit : Icons.fullscreen, color: Colors.white), onPressed: _toggleFullScreen)])]),
              ...
              Expanded(
                child: SingleChildScrollView(
                  ...
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children:[
                      Text("${currentSeason.name} | Episode ${widget.episodeIndex + 1}", style: const TextStyle(color: primaryColor, fontSize: 14, fontWeight: FontWeight.bold)),
                      const SizedBox(height: 4),
                      Text(widget.anime.title, style: const TextStyle(color: Colors.white70, fontSize: 12)),
                      const SizedBox(height: 4),
                      Text(currentEpisode.title, style: const TextStyle(color: Colors.white, fontSize: 22, fontWeight: FontWeight.bold)),
                      const SizedBox(height: 20),
                      Container(
                        ...
                        child: Row(
                          ...
                          children:[
                            GestureDetector(onTap: _toggleLike, child: Row(children:[Icon(isLiked ? Icons.thumb_up : Icons.thumb_up_alt_outlined, color: isLiked ? Colors.orange : Colors.white, size: 22), const SizedBox(width: 8), Text(likes.toString(), style: const TextStyle(color: Colors.white, fontSize: 14, fontWeight: FontWeight.w600))])),
                            Container(width: 1, height: 24, color: Colors.white24), 
                            GestureDetector(onTap: _toggleDislike, child: Row(children:[Icon(isDisliked ? Icons.thumb_down : Icons.thumb_down_alt_outlined, color: isDisliked ? Colors.orange : Colors.white, size: 22), const SizedBox(width: 8), Text(dislikes.toString(), style: const TextStyle(color: Colors.white, fontSize: 14, fontWeight: FontWeight.w600))])),
                            Container(width: 1, height: 24, color: Colors.white24), 
                            GestureDetector(onTap: _toggleSave, child: Row(children:[Icon(_isSaved ? Icons.bookmark : Icons.bookmark_border, color: _isSaved ? Colors.orange : Colors.white, size: 22), const SizedBox(width: 8), Text(_isSaved ? "Saved" : "Save", style: const TextStyle(color: Colors.white, fontSize: 14, fontWeight: FontWeight.w600))])),
                          ],
                        ),
                      ),
                      const SizedBox(height: 30),
...
class _BrowseScreenState extends State<BrowseScreen> {
  ...
  void _submitSearch(String query) {
    if (query.trim().isNotEmpty && !globalRecentSearches.contains(query.trim())) {
      setState(() => globalRecentSearches.insert(0, query.trim()));
    }
    _performSearch(query);
  }
          ...
          child: Column(
            ...
            children:[
              Container(
                ...
                child: TextField(
                  ...
                  decoration: InputDecoration(
                    ...
                    suffixIcon: _searchController.text.isNotEmpty ? IconButton(icon: Icon(Icons.cancel, color: Colors.grey[600]), onPressed: () { _searchController.clear(); _performSearch(""); }) : null,
              ...
              if (_searchController.text.isNotEmpty) ...[
                ...
                const SizedBox(height: 12),
                if (_searchResults.isEmpty) const Center(child: Padding(padding: EdgeInsets.only(top: 20), child: Text("No anime found.", style: TextStyle(color: Colors.white54))))
                ...
                const SizedBox(height: 12),
                if (globalRecentSearches.isEmpty) const Text("No recent searches.", style: TextStyle(color: Colors.white54))
                else ListView.builder(shrinkWrap: true, physics: const NeverScrollableScrollPhysics(), itemCount: globalRecentSearches.length, itemBuilder: (context, index) { return _buildRecentItem(globalRecentSearches[index], index); })
              ]
            ],
          ),
        ),
  ...
  Widget _buildRecentItem(String title, int index) {
    return GestureDetector(
      ...
      child: Container(
        margin: const EdgeInsets.only(bottom: 8), padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 14), decoration: BoxDecoration(color: const Color(0xFF1A1A1A), borderRadius: BorderRadius.circular(10)),
        child: Row(
          mainAxisAlignment: MainAxisAlignment.spaceBetween,
          children:[
            Text(title, style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w600, color: Colors.white)),
            Row(children:[GestureDetector(onTap: () { setState(() { globalRecentSearches.removeAt(index); }); }, child: Icon(Icons.close, size: 18, color: Colors.grey[500])), const SizedBox(width: 12), Icon(Icons.arrow_forward_ios, size: 14, color: Colors.grey[600])]),
          ],
        ),
      ),
    );
    ...
    return DefaultTabController(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          elevation: 0,
          toolbarHeight: 10,
          bottom: const TabBar(indicatorColor: Colors.orange, indicatorWeight: 3, labelColor: Colors.orange, unselectedLabelColor: Colors.white70, tabs:[Tab(text: "DUBBED"), Tab(text: "ORIGINAL")]),
        ),
        body: TabBarView(
          children:[
            GridView.builder(padding: const EdgeInsets.only(top: 20, left: 16, right: 16, bottom: 100), gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 2, childAspectRatio: 0.65, crossAxisSpacing: 14, mainAxisSpacing: 16), itemCount: animeData.length, itemBuilder: (context, index) => GridCategoryCard(anime: animeData[index], pageTitle: "")),
            GridView.builder(padding: const EdgeInsets.only(top: 20, left: 16, right: 16, bottom: 100), gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 2, childAspectRatio: 0.65, crossAxisSpacing: 14, mainAxisSpacing: 16), itemCount: animeData.length, itemBuilder: (context, index) => GridCategoryCard(anime: animeData[index], pageTitle: "")),
          ],
        ),
      ),
...
class _ProfileScreenState extends State<ProfileScreen> {
          ...
          builder: (context, setModalState) {
            return AlertDialog(
              ...
              content: Row(
                children: [
                  Container(
                    padding: const EdgeInsets.symmetric(horizontal: 10), decoration: BoxDecoration(color: Colors.black, borderRadius: BorderRadius.circular(10)),
                    child: DropdownButtonHideUnderline(
                      child: DropdownButton<String>(
                        dropdownColor: Colors.black, value: selectedCountryCode, icon: const Icon(Icons.arrow_drop_down, color: Colors.white54),
                        items: const [DropdownMenuItem(value: "+91", child: Text("🇮🇳 +91", style: TextStyle(color: Colors.white))), DropdownMenuItem(value: "+1", child: Text("🇺🇸 +1", style: TextStyle(color: Colors.white))), DropdownMenuItem(value: "+44", child: Text("🇬🇧 +44", style: TextStyle(color: Colors.white))), DropdownMenuItem(value: "+81", child: Text("🇯🇵 +81", style: TextStyle(color: Colors.white)))],
                        onChanged: (val) { if (val != null) setModalState(() => selectedCountryCode = val); },
                      ),
                    ),
                  ),
                  ...
                  Expanded(
                    child: TextField(
                      controller: _mobileController, keyboardType: TextInputType.phone, style: const TextStyle(color: Colors.white),
                      decoration: InputDecoration(hintText: "Mobile Number", hintStyle: const TextStyle(color: Colors.white38), filled: true, fillColor: Colors.black, border: OutlineInputBorder(borderRadius: BorderRadius.circular(10), borderSide: BorderSide.none)),
                    ),
                  ),
                ],
              ),
              actions: [
                ...
                ElevatedButton(
                  ...
                  onPressed: () {
                    if (_mobileController.text.isNotEmpty) setState(() => addedMobileNumber = "$selectedCountryCode ${_mobileController.text}");
                    Navigator.pop(ctx);
          ...
          child: Column(
            children:[
                ...
                children: [
                  Column(
                    children:[
                      ...
                      const SizedBox(height: 8),
                      GestureDetector(onTap: _showAddInfoDialog, child: Row(children: const [Icon(Icons.add_circle_outline, color: Colors.orange, size: 14), SizedBox(width: 4), Text("Add Info", style: TextStyle(color: Colors.orange, fontWeight: FontWeight.bold, fontSize: 12))])),
                    ],
                  ),
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 4),
                        Text(currentUserEmail, style: const TextStyle(color: Colors.white70, fontSize: 14)),
                        if (addedMobileNumber != null) ...[const SizedBox(height: 8), Row(children:[Text(addedMobileNumber!, style: const TextStyle(color: Colors.grey, fontSize: 12)), const SizedBox(width: 8), const Icon(Icons.check_circle, color: Colors.green, size: 14)])],
                        if (userActivePlan.isNotEmpty) ...[const SizedBox(height: 12), Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4), decoration: BoxDecoration(color: Colors.orangeAccent.withOpacity(0.2), borderRadius: BorderRadius.circular(6), border: Border.all(color: Colors.orangeAccent.withOpacity(0.6))), child: Text(userActivePlan.toUpperCase(), style: const TextStyle(color: Colors.orangeAccent, fontWeight: FontWeight.bold, fontSize: 10, letterSpacing: 1)))],
                      ],
                    ),
                  ),
                ],
              ...
              GestureDetector(
                ...
                child: Container(
                  margin: const EdgeInsets.symmetric(horizontal: 10), padding: const EdgeInsets.all(16), decoration: BoxDecoration(borderRadius: BorderRadius.circular(20), gradient: const LinearGradient(colors: [Colors.orangeAccent, Colors.deepOrange], begin: Alignment.topLeft, end: Alignment.bottomRight)),
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      ...
                      const SizedBox(height: 12),
                      Row(children:[_buildPricePill("₹50"), const SizedBox(width: 8), _buildPricePill("₹100"), const SizedBox(width: 8), _buildPricePill("₹150"), const SizedBox(width: 8), _buildPricePill("₹200")]),
                    ],
                  ),
                ),
              ),
  ...
  Widget _buildPricePill(String price) {
    return Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 6), decoration: BoxDecoration(color: Colors.white.withOpacity(0.2), borderRadius: BorderRadius.circular(8)), child: Text(price, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 11)));
  }
  ...
  Widget _buildMenuItem(BuildContext context, IconData icon, String title, Widget page) {
    return Padding(
      ...
      child: Material(
        color: const Color(0xFF161618), borderRadius: BorderRadius.circular(16),
        child: InkWell(
        ...
        Container(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 12), const Divider(color: Colors.white12, height: 1), const SizedBox(height: 12),
              _buildGridRow("Quality", quality), _buildGridRow("Ads", ads), _buildGridRow("Device Slot", slot),
...
class _PaymentProofPageState extends State<PaymentProofPage> {
  ...
  // --- IN-APP WEBVIEW FOR GOOGLE FORM ---
  late final WebViewController _webViewController;
  final String googleFormLink = "https://docs.google.com/forms/"; // Add your actual link here
  ...
  void initState() {
    ...
    _webViewController = WebViewController()
      ...
      ..loadRequest(Uri.parse(googleFormLink));
  }
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children:[
            ...
            const SizedBox(height: 10),
            Container(
              padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: const Color(0xFF1A1A1A), borderRadius: BorderRadius.circular(12)),
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  ...
                  const SizedBox(height: 8),
                  GestureDetector(
                    onTap: () { _launchUPI(context, _selectedAmount, _selectedPlan); },
                    child: Row(mainAxisAlignment: MainAxisAlignment.center, children: const [Icon(Icons.credit_card, color: Colors.orange), SizedBox(width: 8), Text("Pay via UPI App", style: TextStyle(color: Colors.orange, fontWeight: FontWeight.bold))]),
                  ),
                ],
              ),
            ),
            ...
            SizedBox(
              ...
              child: ElevatedButton.icon(
                style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF1A1A1A), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)), side: const BorderSide(color: Colors.white12)),
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children:[
            Container(
              padding: const EdgeInsets.all(20), decoration: BoxDecoration(borderRadius: BorderRadius.circular(16), gradient: const LinearGradient(colors:[Colors.orangeAccent, Colors.deepOrange])),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: userOrders.isEmpty
          ...
          : ListView.builder(
              padding: const EdgeInsets.all(16), itemCount: userOrders.length,
              itemBuilder: (ctx, i) {
                final order = userOrders[i];
                return Container(
                  margin: const EdgeInsets.only(bottom: 12), padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: const Color(0xFF1A1A1A), borderRadius: BorderRadius.circular(12), border: Border(left: BorderSide(color: order.status == "Pending" ? Colors.orange : Colors.green, width: 4))),
                  child: Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children:[
                      Column(crossAxisAlignment: CrossAxisAlignment.start, children:[Text(order.planName, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 16)), const SizedBox(height: 4), Text("${order.amount} • ${order.date}", style: const TextStyle(color: Colors.white54, fontSize: 12))]),
                      Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4), decoration: BoxDecoration(color: order.status == "Pending" ? Colors.orange.withOpacity(0.2) : Colors.green.withOpacity(0.2), borderRadius: BorderRadius.circular(20)), child: Text(order.status, style: TextStyle(color: order.status == "Pending" ? Colors.orange : Colors.green, fontSize: 12, fontWeight: FontWeight.bold)))
                    ],
                  ),
                );
              },
